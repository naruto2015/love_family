...
//        getParent().requestDisallowInterceptTouchEvent(true);// 用getParent去请求
    ...
    public void close() {
//        Util.showToast(getContext(), "Close");
        Log.e("0012","Close");
        close(true);
    }
    ...
    public void open() {
//        Util.showToast(getContext(), "Open");
        Log.e("0012","Open");
        open(true);
    }
    public void open(boolean isSmooth){
        int finalLeft = -mRange+30;
        if(isSmooth){
            //开始动画
            if(mDragHelper.smoothSlideViewTo(mFrontView, finalLeft, 0)){
                ViewCompat.postInvalidateOnAnimation(this);
    ...
    private void layoutContent(boolean isOpen) {
        ...
        mBottomView.layout(0,0,mBottomView.getRight(),mBottomView.getHeight());
        // 调整顺序, 把mFrontView前置
    ...
    private Rect computeFrontViewRect(boolean isOpen) {
        ...
        return new Rect(left, 0, left + mWidth+30, 0 + mHeight);
    }
    private Rect computeBottomViewViaFront(Rect bottomRect) {
        int hight = bottomRect.height();
        return new Rect(0, 0, 0 , 0 + hight);
    }
    @Override
